...
Function Calling:
  ...
  → 应用执行: get_weather("北京")
  → 返回结果: {"temp": 25, "condition": "晴"}
  → 模型生成: "根据查询结果，北京今天晴天，气温25°C"
```
...
```
1. 应用复杂度
   ├─ 单Agent，简单流程 → Pydantic AI
   └─ 多步骤，复杂工作流 → LangGraph

2. 是否需要Memory？
   ├─ 仅会话内记忆 → Pydantic AI够用
   └─ 跨会话长期记忆 → LangGraph
